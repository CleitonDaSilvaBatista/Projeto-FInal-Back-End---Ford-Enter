...
function aplicarMascaraNumeroCartao(valor){return valor.replace(/\D/g,'').slice(0,16).replace(/(.{4})/g,'$1 ').trim()}
function aplicarMascaraValidade(valor){valor=valor.replace(/\D/g,'').slice(0,4);return valor.length>2?valor.slice(0,2)+'/'+valor.slice(2):valor}
function atualizarPreviewCartao(){const preview=$('cartaoPreview');if(!preview)return;preview.style.display='block';$('previewNumero').textContent=aplicarMascaraNumeroCartao($('numeroCartao')?.value||'')||'0000 0000 0000 0000';$('previewTitular').textContent=($('nomeTitular')?.value||'NOME DO TITULAR').toUpperCase();$('previewValidade').textContent=$('validadeCartao')?.value||'00/00'}
function getDonoCartao(){return usuarioAtual?.email||''}function getCartoesDoUsuarioAtual(){const dono=getDonoCartao();return JSON.parse(localStorage.getItem('cartoes')||'[]').filter(c=>c.usuario===dono)}
async function cadastrarCartao(event){event?.preventDefault();try{const body={nomeTitular:$('nomeTitular').value.trim(),numero:$('numeroCartao').value.trim(),validade:$('validadeCartao').value.trim(),cvv:$('cvvCartao').value.trim(),limite:converterMoedaParaNumero($('limiteCartao').value)};if(!body.nomeTitular||!body.numero||!body.validade||!body.cvv)throw new Error('Preencha todos os campos do cartão.');const cartoes=JSON.parse(localStorage.getItem('cartoes')||'[]');body.id=Date.now();body.usuario=getDonoCartao();cartoes.push(body);localStorage.setItem('cartoes',JSON.stringify(cartoes));$('cartaoForm').reset();await listarCartoes();toast('Cartão cadastrado','Novo cartão salvo com sucesso.')}catch(e){toast('Erro ao cadastrar cartão',e.message)}}
async function listarCartoes(){try{const cartoes=getCartoesDoUsuarioAtual();const area=$('listaCartoes');if(!area)return;if(!cartoes.length){area.innerHTML='<div class="empty-state"><p>Nenhum cartão cadastrado.</p></div>';return}area.innerHTML=cartoes.map(c=>`<article class="account-card"><div><h3>${c.nomeTitular}</h3><div class="account-meta"><span class="tag"><i class="bi bi-credit-card"></i>${aplicarMascaraNumeroCartao(c.numero||'')}</span><span class="tag">Validade ${c.validade}</span></div><div class="account-actions"><button class="icon-btn" onclick="excluirCartao('${c.id}')"><i class="bi bi-trash"></i></button></div></div><div class="balance">${money.format(Number(c.limite||0))}</div></article>`).join('')}catch(e){toast('Erro ao listar cartões',e.message)}}
function excluirCartao(id){if(!confirm('Deseja excluir este cartão?'))return;const dono=getDonoCartao();const cartoes=JSON.parse(localStorage.getItem('cartoes')||'[]').filter(c=>!(String(c.id)===String(id)&&c.usuario===dono));localStorage.setItem('cartoes',JSON.stringify(cartoes));listarCartoes();toast('Cartão excluído','O cartão foi removido com sucesso.')}
